classdef den_PowerAmplifier
    ...
    methods
        function obj = PowerAmplifier(order, memory_depth, input, output)
...
%             default_poly_coeffs = [ 1.0513 + 0.0904i, -0.0680 -0.0023i, 0.0289 - 0.0054i;  ...% 1st order coeffs
%                                     -0.0542 - 0.2900i, 0.2234 + 0.2317i, -0.0621 - 0.0932i; ...% 3rd order coeffs
%                                     -0.9657 - 0.7028i, -0.2451 - 0.3735i, 0.1229 + 0.1508i;...% 5th order coeffs
%                                     0.1774 + 0.0265i, 0.0848 + 0.0613i, -0.0362 - 0.0307i]; % 7th order coeffs
            % Prune the model to have the desired number of nonlinearities and memory effects.
            
            for i =1:length(input)
                p = polyfit(input, output, 4);
                % f_model = polyval(p,tx_data);
                syms a b c d e x

                f_syms = a*x+b*y+c*z+d*w+e ;
                f_syms_inv = finverse(f_syms,x);
                ...
                x = input;
                p_dpd_out = eval(f_syms_inv);
                % plot(sort(pin),sort(f_model))
                % hold on
                % plot(sort(pin),sort(abs(p_dpd_out)),'r')
               
                x = p_dpd_out;
                pa_out_pp = eval(f_syms);
            end
